...
#         Validations.valid_status_code(response, 200)
#         Validations.check_json_keys(response, ["id", "token"])
#         Validations.response_time(response, 1000)
#         print("Successful POST request")
#
#     def test_login(self):
#         response = ReqresInApi.login("eve.holt@reqres.in", "Qwerty")
#         Validations.valid_status_code(response, 200)
#         Validations.check_json_keys(response, ["token"])
#         Validations.response_time(response, 1000)
#         print("Successful POST request")
...
class TestSuitNegative:
    ...
    def test_registration_without_email(self):
        ...
        Validations.check_json_keys(response, ["error"])
        Validations.check_json_values(response, ["Missing email or username"])
        Validations.response_time(response, 8000)
    ...
    def test_registration_without_password(self):
        ...
        Validations.valid_status_code(response, 400)
        Validations.check_json_keys(response, ["error"])
        Validations.check_json_values(response, ["Missing password"])
        Validations.response_time(response, 8000)
